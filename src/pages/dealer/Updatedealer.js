...
import { createdealer, getdealerbyid, updatedealer } from "../../slices/dealer";
...
const UpdateDealer = ({ setLoader, show }) => {
  ...
  React.useEffect(()=>{
    ...
    if(dealerdata.region){
      const zoneData = settingData.region?.filter((va)=>{return va.name === dealerdata?.region});
          setZones(JSON.parse(zoneData[0]?.zones));
    }
  },[settingData, dealerdata]);

  const regionData = region.map((va)=>{return va.name});
  ...
  const handleRegion1 = (ab) => {
    const zoneData = settingData.region?.filter((va)=>{return va.name === ab});
    setZones(JSON.parse(zoneData[0].zones));
}
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const formData = {...dealerdata, billing_address: JSON.stringify(billing), shipping_address:JSON.stringify(shipping), bank_details:JSON.stringify(bank), documents: JSON.stringify(doc),
    contact_person: JSON.stringify(dealerdata.contact_person)};
    const dealer_id = params.dealerId
    setLoader(true)
    ...
    onSubmit={handleSubmit}>
        <Grid container spacing={3} sx={{ paddingBottom: 2, alignItems: "center" }}>
          ...
          >
            ...
            <Typography variant="h5"  sx={{fontWeight:'bold'}}>Update Dealer</Typography>
          </Grid>
          ...
          onChange={(e) => handleFieldChange('salutation', ev.field_id, e)}        >
            ...
            <Grid container spacing={2}>
                <Grid item xs={12} sx={{textAlign:'end'}}>
                  ...
                  <Button type="submit" variant="contained">Update Dealer</Button>
                </Grid>
            </Grid>
      </Box>
    </div>
